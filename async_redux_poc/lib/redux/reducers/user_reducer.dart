...
AppState userReducer(AppState state, dynamic action) {
  if (action is FetchUserRequestAction) {
    ...
    return state.copyWith(userFetchState: Loading());
  }
  ...
  if (action is FetchUserSuccessAction) {
    ...
    return state.copyWith(userFetchState: Loaded(action.user));
  }
  ...
  if (action is FetchUserFailureAction) {
    ...
    return state.copyWith(userFetchState: FetchError(action.errorMessage));
  }
